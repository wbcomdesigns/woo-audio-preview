...
class Wbcom_Settings_Page {
	...
	public static function render_welcome() {
		...
		<div class="wrap wbcom-admin">
			<header class="wbcom-page-header">
				...
				<div>
					<h1><?php esc_html_e( 'WB Plugins', 'woocommerce-price-quote' ); ?></h1>
					<p class="wbcom-page-header__subtitle">
						<?php
						echo esc_html(
							sprintf(
								/* translators: %d: number of active Wbcom plugins. */
								_n( '%d Wbcom plugin active on this site.', '%d Wbcom plugins active on this site.', $count, 'woocommerce-price-quote' ),
								$count
							)
						);
						?>
			...
			<?php if ( 0 === $count ) : ?>
				<div class="wbcom-empty-state">
					...
					<p class="wbcom-empty-state__title"><?php esc_html_e( 'No Wbcom plugins attached to this hub yet', 'woocommerce-price-quote' ); ?></p>
					<p class="wbcom-empty-state__desc"><?php esc_html_e( 'Activate one and it will appear here automatically.', 'woocommerce-price-quote' ); ?></p>
				</div>
			<?php else : ?>
					...
					<?php foreach ( $plugins as $plugin ) : ?>
						<a class="wbcom-hub-card" href="<?php echo esc_url( $plugin['url'] ); ?>">
							...
							<span class="wbcom-hub-card__cta">
								<?php esc_html_e( 'Open settings', 'woocommerce-price-quote' ); ?>
			...
			<?php
			self::card_open( __( 'About WB Plugins', 'woocommerce-price-quote' ) );
			?>
			<p>
				<?php esc_html_e( 'This hub is the single entry point for every Wbcom Designs plugin installed on your site. Each plugin lives on its own page under this menu and keeps its own settings, licence, and data.', 'woocommerce-price-quote' ); ?>
			</p>
			<p>
				<a href="https://wbcomdesigns.com/" target="_blank" rel="noopener noreferrer">
					<?php esc_html_e( 'Visit wbcomdesigns.com for more plugins and themes', 'woocommerce-price-quote' ); ?>
					<i data-lucide="arrow-right"></i>
				</a>
			</p>
			<?php
	...
	public static function render() {
		...
		<div class="wrap wbcom-admin">
			 ...
			 * "Settings saved" banner inside whichever section owned the first
			 * heading - the Quote Requests tab - instead of at the top of the
			 * page the owner is actually on. A real header with the marker after
			 * it keeps the notice where it belongs, on every tab.
			 */
			?>
